...
R_OFDM = real(OFMD_mod);
I_OFDM = imag(OFMD_mod);
...
symb_utiles_RX = reshape(z, [64,4]);
OFMD_mod_RX = fft(symb_utiles_RX);
OFMD_mod_RX = reshape (OFMD_mod_RX , [1,256]);

R_OFDM = real(OFMD_mod_RX);
I_OFDM = imag(OFMD_mod_RX);

figure(400), plot(R_OFDM,I_OFDM,'*');
