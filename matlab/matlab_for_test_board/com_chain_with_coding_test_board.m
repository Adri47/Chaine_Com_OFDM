...
%% Read UART BCH Encoder
% C_hard = recv_UART(s, bch_bit_nb);
% C_hard = reshape((double(dec2bin(C_hard))-48)',[7,32])';
% X_soft= C_hard  

%% Write UART
% X_soft_reshape = reshape(X_soft', 1,[]);
% X_soft_size = length(X_soft_reshape);
% s = send_UART(X_soft_reshape,X_soft_size);

%% Interleaver
% P_soft=convintrlv([reshape(X_soft.',1,[])],intlvr_line_nb,intlvr_reg_size);

%% Read UART
% S_hard=recv_UART(s, 224);
% P_soft = S_hard;
